...
children.push(
  T(
    [
      ...
      ['16', 'File khoan không khai định dạng số (co 10 lần) hoặc lệch gốc so với Gerber', 'FRIWO — 55807.931-90FE (+ 302 bộ / 18.496 bộ)', 'Đã sửa · c680077, 3ec37c4'],
...
const bugs = [
  ...
  },
  {
    ...
    rows: [
      ...
      ['Kiểm', 'FRIWO: 2641 lỗ về đúng vùng bo, kích thước giữ 137.4 × 147 mm; PCB_doline, The Cold dời lại khớp 100% pad. Hồi quy corpus, đo bằng **tỉ lệ lỗ trúng pad**: mẫu 1/10 (1.850 bộ) — **31 bộ đổi, cả 31 tốt lên, 0 tệ đi, không đổi kích thước**; bộ trúng pad < 50% giảm 34 → 3. 21 bộ sai định dạng, 10 bộ lệch gốc. Bản chỉ dùng khung bo (c680077) sửa được 10. **Quét toàn kho (18.496 zip): 314 bộ đổi — 302 tốt lên, 0 tệ đi**; bộ trúng pad < 50% giảm 315 → 23; 8 bộ đổi kích thước không do phần khoan (sửa viền ở commit khác hoặc lệch khi chạy chung). Chốt thêm: khung bo làm thước chỉ nhận khi trúng pad tăng ≥ 20 điểm; hình > 20 mm không tính là pad; file khoan ngoài bo không cách đọc nào khớp thì cảnh báo đỏ. 6 test mới.'],
  ...
  },
  {
    ...
    rows: [
      ...
      ['Treo app', 'P84390-S02 có lớp in lụa 150.220 vùng tô: renderThree dựng mỗi hình một khối nên ra 180 triệu đỉnh, hết bộ nhớ và mất 198 s, app đứng hình. Sửa: lớp > 50.000 hình tự ráp MỘT lưới tam giác cho cả lớp (bf511b8, fastLayer.ts). Lụa mặt dưới 85.570 hình: 102,5 triệu đỉnh → 758.766 đỉnh; mở cả bộ 75,3 s → 9,0 s; bộ nhớ trang 4,5 GB → 1,2 GB. Vẽ ĐỦ cả hai lớp lụa ở cả bốn chế độ xem, phóng to đọc được chữ. Đánh đổi: hình phẳng, bỏ hình đảo cực. Sửa tiếp 2316510: vùng tô lõm phải cắt tai chứ không chia quạt (chia quạt làm đầy ruột chữ — diện tích gấp 3,6 lần thật, chữ bết thành cục) và lưới phải bọc trong Group (three xếp thứ tự vẽ theo groupOrder trước, Mesh trần bị mask/đồng đè nên 2 Mặt / 2D / 3D không thấy in lụa).'],
      ...
      ['Còn lại', 'Lụa vẽ ở chế độ nhẹ là hình phẳng, không có bề dày như lớp thường — chỉ để xem, không dùng để đo.'],
    ],
  },
]
