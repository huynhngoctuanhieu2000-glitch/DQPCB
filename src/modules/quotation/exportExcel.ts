...
export const buildQuotationWorkbook = (q: Quotation): ExcelJS.Workbook => {
  ...
  ws.getCell('D1').value = {
    richText: [
      { font: { name: FONT, size: 18, bold: true, color: { argb: COMPANY_RED } }, text: `${q.company.name}\r\n` },
      { font: { name: FONT, size: 14, bold: true, color: { argb: 'FF000000' } }, text: `${q.company.address}\r\n` },
      { font: { name: FONT, size: 14, bold: true, color: { argb: 'FF000000' } }, text: q.company.contact },
    ],
  }
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    if (item?.discount) {
      ...
      style(ws.getCell(r, 1), { text: item.name, size: 12, bold: true, wrap: true })
  ...
  style(ws.getCell(sigFoot, 9), { text: q.preparedBy, size: 13, bold: true, color: RED, align: 'center' })
